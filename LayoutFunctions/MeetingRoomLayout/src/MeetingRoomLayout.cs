...
namespace MeetingRoomLayout
{
    ...
    public static class MeetingRoomLayout
    {
        private class MeetingLayoutGeneration : LayoutGeneration<LevelElements, LevelVolume, SpaceBoundary, CirculationSegment, SpaceSettingsOverride, SpaceSettingsValue>
        {
            ...
            public override LayoutGenerationResult StandardLayoutOnAllLevels(string programTypeName, Dictionary<string, Model> inputModels, dynamic overrides, bool createWalls, string configurationsPath, string catalogPath = "catalog.json", Func<SpaceSettingsOverride, Vector3> getCentroid = null, SpaceSettingsValue defaultValue = null)
            {
                seatsTable = new Dictionary<string, RoomTally>();
                var result = base.StandardLayoutOnAllLevels(programTypeName, inputModels, (object)overrides, createWalls, configurationsPath, catalogPath, getCentroid, defaultValue);
        ...
        public static MeetingRoomLayoutOutputs Execute(Dictionary<string, Model> inputModels, MeetingRoomLayoutInputs input)
        {
            ...
            var result = layoutGeneration.StandardLayoutOnAllLevels("Meeting Room", inputModels, input.Overrides, input.CreateWalls, "./ConferenceRoomConfigurations.json", default, (ov) => ov.Identity.ParentCentroid, new SpaceSettingsValue(false, false));
