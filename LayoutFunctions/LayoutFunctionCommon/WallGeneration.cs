            ...
            foreach (var wallCandidate in wallCandidateLines)
            {
                ...
                else if (wallCandidate.type == "Glass")
                {
                    ...
                    foreach (var separator in separators)
                    {
                        ...
                        var instance = mullion.CreateInstance(new Transform(separator, wallCandidate.line.Direction(), Vector3.ZAxis, 0).Concatenated(levelTransform), "Mullion");
                        model.AddElement(instance);
                    }
                    model.AddElement(new StandardWall(wallCandidate.line, 0.05, totalStorefrontHeight, glassMat, levelTransform));
                    var headerHeight = height - totalStorefrontHeight;
                    if (headerHeight > 0.01)
                    {
                        model.AddElement(new StandardWall(wallCandidate.line, 0.2, headerHeight, wallMat, levelTransform.Concatenated(new Transform(0, 0, totalStorefrontHeight))));
                    }
                }
            }
        }
